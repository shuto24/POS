...
bp = Blueprint("kakeibo", __name__, url_prefix="/kakeibo")
...
def add():
    if request.method == "POST":
        ...
        return redirect(url_for("kakeibo.index", month=date[:7]))
...
def delete(id_):
    ...
    return redirect(url_for("kakeibo.index", month=month))
...
def budget_save():
    ...
    return redirect(url_for("kakeibo.budget", year=year))


@bp.route("/budget/delete", methods=["POST"])
def budget_delete():
    year = int(request.form["year"])
    type_ = request.form["type"]
    category = request.form["category"]
...
def categories_add():
    ...
    return redirect(url_for("kakeibo.categories"))
